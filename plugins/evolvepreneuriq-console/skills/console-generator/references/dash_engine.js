...
(function(){
  ...
  /* ---- defensive helpers: the generator is expected to inject the full   ---- */
  /* ---- documented schema, but a live data pull can come back partial —   ---- */
  /* ---- never let a missing/odd field crash the tab or print "$NaN".      ---- */
  function numOr(v,d){ v=Number(v); return isFinite(v)?v:d; }
  function arrOr(v,d){ return Array.isArray(v)?v:d; }
  function pct(n,d){ n=Number(n); d=Number(d); return (isFinite(n)&&isFinite(d)&&d>0)?Math.round(100*n/d):0; }
  function safeDiv(n,d,fallback){ n=Number(n); d=Number(d); return (isFinite(n)&&isFinite(d)&&d>0)?(n/d):(fallback||0); }

  /* normalize one EIQ tenant record in place — fills gaps, and demotes a   */
  /* "has.X" flag to false if the figures needed to render X aren't usable, */
  /* so the UI falls back to the next-best KPI instead of crashing/NaN.     */
  function normEiqBrand(d){
    if(!d||typeof d!=='object') return d;
    d.has = d.has||{};
    var r = d.rev = d.rev||{};
    var financeOk = !!(d.has.finance && d.finance && isFinite(Number(r.fyRev)) && isFinite(Number(r.fyNet))
                        && arrOr(d.finance.revTop,[]).length && arrOr(d.finance.expTop,[]).length);
    d.has.finance = financeOk;
    r.fyRev=numOr(r.fyRev,0); r.fyExp=numOr(r.fyExp,0); r.fyNet=numOr(r.fyNet,0);
    r.ytd=numOr(r.ytd,0); r.ytdOrders=numOr(r.ytdOrders,0);
    r.lastMonth=numOr(r.lastMonth,0); r.lastMonthOrders=numOr(r.lastMonthOrders,0);
    if(financeOk){ d.finance.revTop=arrOr(d.finance.revTop,[]); d.finance.expTop=arrOr(d.finance.expTop,[]); }

    var c = d.cust = d.cust||{};
    c.total=numOr(c.total,0); c.avgLtv=numOr(c.avgLtv,0); c.oneOff=numOr(c.oneOff,0);
    c.oneOffPct=numOr(c.oneOffPct,0); c.totalLtv=numOr(c.totalLtv,0);
    c.top = arrOr(c.top,[]).length?c.top:[['—',0,0]];
    c.newByYear = (c.newByYear&&arrOr(c.newByYear.l,null))?c.newByYear:{l:[],v:[]};
    c.nvr = (c.nvr&&arrOr(c.nvr.l,null))?c.nvr:{l:[],nw:[],rt:[]};
    c.nvr.nw=arrOr(c.nvr.nw,[]); c.nvr.rt=arrOr(c.nvr.rt,[]); c.nvr.l=arrOr(c.nvr.l,[]);
    c.ltvBuckets = (c.ltvBuckets&&arrOr(c.ltvBuckets.l,null))?c.ltvBuckets:{l:[],v:[]};
    c.ordBuckets = (c.ordBuckets&&arrOr(c.ordBuckets.l,null))?c.ordBuckets:{l:[],v:[]};

    d.has.pipeline = !!(d.has.pipeline && d.pipeline && arrOr(d.pipeline.stages,[]).length && isFinite(Number(d.pipeline.total)));
    if(d.has.pipeline){ var dl=d.pipeline; dl.total=numOr(dl.total,0); dl.pipelineValue=numOr(dl.pipelineValue,0); dl.openValue=numOr(dl.openValue,0); dl.openCount=numOr(dl.openCount,0); dl.wonValue=numOr(dl.wonValue,0); dl.lostValue=numOr(dl.lostValue,0); dl.winRateVal=numOr(dl.winRateVal,0); dl.stages=arrOr(dl.stages,[]); }

    d.has.subs = !!(d.has.subs && d.subs && numOr(d.subs.active,0)>0 && isFinite(Number(d.subs.mrr)));
    if(d.has.subs){ var su=d.subs; su.active=numOr(su.active,0); su.mrr=numOr(su.mrr,0); su.ccy=su.ccy||'AUD'; su.churn=numOr(su.churn,0); }

    d.has.royalties = !!(d.has.royalties && d.roy && arrOr(d.roy.platforms,[]).length && isFinite(Number(d.roy.gross)));
    if(d.has.royalties){ var ro=d.roy; ro.gross=numOr(ro.gross,0); ro.supplier=numOr(ro.supplier,0); ro.units=numOr(ro.units,0); ro.txns=numOr(ro.txns,0); ro.payTotal=numOr(ro.payTotal,0); ro.payCount=numOr(ro.payCount,0); ro.platforms=arrOr(ro.platforms,[]); }
    return d;
  }
  function normTraffic(t){
    if(!t||!arrOr(t.ch,[]).length||!arrOr(t.pages,[]).length) return null;
    t.visits=numOr(t.visits,0); t.unique=numOr(t.unique,0); t.bounce=numOr(t.bounce,0); t.dur=numOr(t.dur,0); t.ret=numOr(t.ret,0);
    return t;
  }
  function normEpBrand(b){
    if(!b||typeof b!=='object'||b.empty) return b;
    b.k = b.k||{};
    ['rev','rev12','orders','cust','members','new12','enrol','courses','books','recur'].forEach(function(f){ b.k[f]=numOr(b.k[f],0); });
    b.annual = (b.annual&&arrOr(b.annual.l,null))?b.annual:{l:[],v:[]};
    b.monthly = (b.monthly&&arrOr(b.monthly.l,null))?b.monthly:{l:[],v:[]};
    b.customers = arrOr(b.customers,[]).length?b.customers:[['—','—',0,0]];
    b.color = b.color||'#6d28d9';
    return b;
  }
  if(EIQ) Object.keys(EIQ).forEach(function(k){ if(k!=='group') normEiqBrand(EIQ[k]); });
  if(TR) Object.keys(TR).forEach(function(k){ TR[k]=normTraffic(TR[k]); });
  if(BR) Object.keys(BR).forEach(function(k){ if(k!=='group') normEpBrand(BR[k]); });
  if(EIQGROUP){ EIQGROUP.totalCust=numOr(EIQGROUP.totalCust,0); EIQGROUP.totalLtv=numOr(EIQGROUP.totalLtv,0); EIQGROUP.brands=arrOr(EIQGROUP.brands,[]); }
  if(GROUP){
    ['rev','orders','cust','members','visits','rev12'].forEach(function(f){ GROUP[f]=numOr(GROUP[f],0); });
    GROUP.labels=arrOr(GROUP.labels,[]); GROUP.revLife=arrOr(GROUP.revLife,[]); GROUP.colors=arrOr(GROUP.colors,[]); GROUP.visits12=arrOr(GROUP.visits12,[]);
  }
  ...
  /* ---- helpers ---- */
  function money(n){ n=Number(n); return isFinite(n)?('$'+Math.round(n).toLocaleString()):'—'; }
  function moneyK(n){ n=Number(n); if(!isFinite(n))return '—'; return Math.abs(n)>=1000?'$'+(n/1000).toFixed(Math.abs(n)>=100000?0:1)+'k':'$'+Math.round(n); }
  function num(n){ n=Number(n); return isFinite(n)?n.toLocaleString():'—'; }
  ...
  function renderDashBody(){
    ...
    try{
      ...
      emptyBrand(ws,'Couldn’t render this view','This section’s data came back in a shape I didn’t expect, so I stopped rather than show something wrong. Ask me to “refresh my dashboard from EIQ” to re-pull it, or try a different business/section from the menu.'+(window.console?(console.error('Dashboard render error:',e),''):''));
    }
  }
  ...
  function renderEiqGroup(ws){
    ...
    ws.innerHTML=head('Group Overview','Consolidated across every tenant whose EiQ connector is live. Money is settled AUD (customer-analytics lifetime).')+
      ...
      '<div class="dk-kpis">'+
        ...
        kpi('Biggest tenant',top[1],money(top[2])+' · '+pct(top[2],g.totalLtv)+'%')+
        kpi('Avg lifetime / customer',money(safeDiv(g.totalLtv,g.totalCust)))+
      '</div>'+
      '<div class="dk-g2">'+
  ...
  function renderEiqBrand(ws,brand,s){
    ...
    if(s==='overview'){
      ...
      var top=c.top[0], share=pct(top[2],c.totalLtv);
      ...
      ws.innerHTML=head('Royalties & Books','Book royalty sales across every distribution platform.')+
        ...
        '<div class="dk-note">Top platform: <b>'+top[0]+'</b> — '+money(top[1])+' gross ('+pct(top[1],r.gross)+'% of the total) and '+num(top[2])+' units.</div>'+
        '<div class="dk-g2">'+
  ...
  function renderTraffic(ws,brand){
    ...
    ws.innerHTML=head('Marketing & Traffic','Website traffic from EIQ Analytics (the platform’s Matomo store).')+
      ...
      '<div class="dk-kpis">'+
        ...
        kpi('Returning visitors',num(t.ret),pct(t.ret,t.visits)+'% of traffic')+
    ...
    if(id==='overview'){
      ws.innerHTML=head('Overview','Headline numbers for this brand.')+
        '<div class="dk-kpis">'+
          ...
          kpi('Paying customers',num(b.k.cust),'AOV '+money(safeDiv(b.k.rev,b.k.orders)))+
      ...
      ws.innerHTML=head('Sales & Revenue','Revenue, orders and the products driving them.')+
        '<div class="dk-kpis">'+kpi('Lifetime paid revenue',money(b.k.rev),null,1)+kpi('Revenue — last 12 mo',money(b.k.rev12))+kpi('Recurring revenue',money(b.k.recur))+kpi('Paid orders',num(b.k.orders))+kpi('Avg order value',money(safeDiv(b.k.rev,b.k.orders)))+'</div>'+
      ...
      ws.innerHTML=head('Members & Courses','Registered members, courses and enrolments.')+
        '<div class="dk-kpis">'+kpi('Registered members',num(b.k.members),null,1)+kpi('New members — 12 mo',num(b.k.new12))+kpi('Courses',num(b.k.courses))+kpi('Enrolments',num(b.k.enrol))+kpi('Enrolments / member',safeDiv(b.k.enrol,b.k.members).toFixed(2))+'</div>'+
      ...
      var top=b.customers[0], share=pct(top[3],b.k.rev), t8=pct(b.customers.reduce(function(a,c){return a+numOr(c[3],0);},0),b.k.rev);
      ws.innerHTML=head('Top Customers','Who spends the most on this brand.')+
        '<div class="dk-kpis">'+kpi('Top customer',(top[0]||'—').split(' ')[0]+'…',money(top[3]),1)+kpi("Top customer's share",share+'%')+kpi('Top 8 share',t8+'%')+kpi('Paying customers',num(b.k.cust))+kpi('Avg lifetime value',money(safeDiv(b.k.rev,b.k.cust)))+'</div>'+
